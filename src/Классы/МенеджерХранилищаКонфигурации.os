...
Процедура СохранитьВерсиюКонфигурацииРасширенияВФайл(Знач НомерВерсии, Знач ИмяФайлаКофигурации, Знач Расширение) Экспорт
	...
	Если Не ПустаяСтрока(Расширение) Тогда
        Параметры.Добавить("-Extension "+Расширение);
    КонецЕсли;
...
//
Процедура ПодключитьсяКХранилищу(Знач ИгнорироватьНаличиеПодключеннойБД = Ложь, Знач ЗаменитьКонфигурациюБД = Истина, ИмяРасширения = "") Экспорт
...
//
Процедура ОтключитьсяОтХранилища(ИмяРасширения = "") Экспорт
    Параметры = СтандартныеПараметрыЗапуска();
